...
class LocationService {
    ...
    // 주변 거점의 모든 물품 확인
    func getPostList(_ latitude: Double, _ longitude: Double, _ latitudeDelta: Double, _ longitudeDelta: Double, completion: @escaping (_ postListByLocation: PostListByLocation) -> Void) {
        ...
        dataRequest.responseDecodable(of: BaseResponse<PostListByLocation>.self) { response in
    ...
    // 특정 거점 물품 확인
    func getPlacePostList(_ locationId: Int, completion: @escaping (_ postListByLocation: PlacePostList) -> Void) {
        ...
        dataRequest.responseDecodable(of: BaseResponse<PlacePostList>.self) { response in
            ...
            case .failure(let error):
                print("DEBUG(get place post list api) error: \(error)")
            }
        }
